...
class BusinessLogic:
    ...
    def __init__(self) -> None:
        self.data_analyzer = DataAnalyzer(window=50)
        self.data_analyzer.ai_manager.load_model()

    def get_predicted_change(self, symbol: str, last_date: datetime.datetime) -> tuple[datetime.date, tuple[float, float, float, float, float, float]]:
        """
        ...
        Returns:
            tuple[last real, real percentage change, real value change, last predicted, predicted percentage change, predicted value change]
        """
        result = [0, 0, 0, 0, 0, 0]
