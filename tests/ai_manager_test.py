...
class AIManagerTests(unittest.TestCase):
    ...
    def creation_and_scaling(self, symbol:str):
        ...
        data_manager.columns_to_drop.clear()
        ...
        self.assertTrue(np.array_equal(np.round(original_3,decimals=6), np.round(scaled_back[:,0], decimals=6)))
        self.assertTrue(np.array_equal(np.round(original_0,decimals=6), np.round(scaled_back[:,1], decimals=6)))
        self.assertTrue(np.array_equal(np.round(original_1,decimals=6), np.round(scaled_back[:,2], decimals=6)))
        self.assertTrue(np.array_equal(np.round(original_2,decimals=6), np.round(scaled_back[:,3], decimals=6)))
        ...
        # scaled_back = ai_manager.scale_back_array(values=scaled)
        # print(f'Original: {np.round(original_3,decimals=6)} Predicted: {np.round(scaled_back, decimals=6)}')
    ...
    def test_prediction(self):
        ...
        ai_manager.create_model(shape=(ai_manager.window, 6-len(data_manager.columns_to_drop)))
